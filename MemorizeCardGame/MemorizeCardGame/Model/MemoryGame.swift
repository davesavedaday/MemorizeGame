...
struct MemoryGame<CardContent> where CardContent: Equatable{
    ...
    var score: Int
    var listOfFlippedCards: Set<Int>
    
    mutating func choose(card: Card) {
        print("card choosen: \(card)")
        if let chosenIndex: Int = cards.firstIndex(matching: card), !cards[chosenIndex].isFaceUp, !cards[chosenIndex].isMatched {
            if let potentialMatchingIndex = indexOfTheOneAndOnlyFaceUpCard {
                if(cards[potentialMatchingIndex].content == cards[chosenIndex].content) {
                    cards[potentialMatchingIndex].isMatched = true
                    cards[chosenIndex].isMatched = true
                    score += 2
                }
                else {
                    if listOfFlippedCards.contains(cards[chosenIndex].id){
                        score -= 1
                    }
                    if listOfFlippedCards.contains(cards[potentialMatchingIndex].id) {
                        score -= 1
                    }
                    listOfFlippedCards.insert(cards[chosenIndex].id)
                    listOfFlippedCards.insert(cards[potentialMatchingIndex].id)
                }
